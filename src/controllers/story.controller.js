...
const listStoryEventsHandler = asyncHandler(async (req, res) => {
  ...
  const events = await getAvailableEvents(storyInstance, playerState, countryState);
...
const submitChoiceHandler = asyncHandler(async (req, res) => {
  ...
  const choiceIndex = Number(req.body.choice_index != null ? req.body.choice_index : req.body.choiceIndex);

  if (!storyId || !eventId || !Number.isInteger(choiceIndex) || choiceIndex < 0) {
    throw new AppError('story_id, event_id, and choice_index are required', 400);
  }
...
const getPlayerStateHandler = asyncHandler(async (req, res) => {
  const playerState = await getOrCreatePlayerState(req.user.id, req.user);
  res.json(serializePlayerState(playerState));
});
